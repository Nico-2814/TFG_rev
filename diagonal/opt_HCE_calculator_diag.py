...
from qutip import qsave, Qobj, entropy_vn
from os import makedirs

makedirs("HCEs", exist_ok=True)
...
H_grid = np.empty((4, 4, N_points, N_points))
for i in range(4):
    for j in range(4):
        H_grid[i, j] = res[i][j]
...
for beta in betas:
    ...
    for m in range(NQ2):
        for mp in range(NQ2):
            print(m, mp)
            B = np.einsum('noi,ij,mpj->nmop', phi_prod, rho_xi[m, mp], phi_prod) * Delta2
            B_2d = B.reshape(Nc2, Nc2)
            del B
            rho_HCE[Nc2*m:Nc2*(m+1), Nc2*mp:Nc2*(mp+1)] = B_2d
    '''

    from joblib import Parallel, delayed #lo mismo pero paralelizado

    def calcular_bloque_einsum(m, mp, rho_val, phi_p, D2, N2):
        print(f"Procesando: m={m}, mp={mp}")
        B = np.einsum('noi,ij,mpj->nmop', phi_p, rho_val, phi_p) * D2
           
        return m, mp, B.reshape(N2, N2)


    resultados = Parallel(n_jobs=4)(
        delayed(calcular_bloque_einsum)(m, mp, rho_xi[m, mp], phi_prod, Delta2, Nc2) 
    ...
    print("Entropía: ", entropy_vn(rho_qobj))
    qsave(rho_qobj, f"HCEs/HCE_{beta}_{N_c}_diag")