...
k_val = 0.5
makedirs(f"molecular_model/HCEs/k_{k_val}/Nc", exist_ok=True)
with open("molecular_model/HCEs/entropies.csv", "a") as f:
    q_max = 10 
    N_points = 3000 
    Delta = 2*q_max/(N_points-1)
    for N_c in range(5,50, 2):
        print(f"Nc: {N_c}")
        def phi_n(n, x):
        ...
        for beta in betas:
            ...
            f.write(f"{N_c},{entropy_vn(rho_qobj/rho_qobj.tr())}\n")
            f.flush()
            if np.isclose(beta, 1.0, atol=1e-4):
                ...
                fig.savefig(f"molecular_model/HCEs/k_{k_val}/Nc/HCE_{N_c}.png")
